      ...
      <h2 id="document-heading" data-cy="DocumentHeading">
        ...
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            ...
            <Translate contentKey="eCompanyApp.document.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/document/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="eCompanyApp.document.home.createLabel">Create new Document</Translate>
          </Link>
        </div>
      </h2>
        ...
        {documentList && documentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="eCompanyApp.document.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('uploadFile')}>
                  <Translate contentKey="eCompanyApp.document.uploadFile">Upload File</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('uploadFile')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {documentList.map((document, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`eCompanyApp.DocumentType.${document.documentType}`} />
                  </td>
                  ...
                  <td>{document.expiryDate ? <TextFormat type="date" value={document.expiryDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
                    <Translate contentKey={`eCompanyApp.DocumentStatus.${document.documentStatus}`} />
                  </td>
                  <td>
                    {document.uploadFile ? (
                      <div>
                        {document.uploadFileContentType ? (
                          <a onClick={openFile(document.uploadFileContentType, document.uploadFile)}>
                            <Translate contentKey="entity.action.open">Open</Translate>
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {document.uploadFileContentType}, {byteSize(document.uploadFile)}
                        </span>
                      </div>
                    ) : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="eCompanyApp.document.home.notFound">No Documents found</Translate>
            </div>
          )
        )}
      </div>
