      ...
      <h2 id="bank-detail-heading" data-cy="BankDetailHeading">
        ...
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            ...
            <Translate contentKey="eCompanyApp.bankDetail.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/bank-detail/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="eCompanyApp.bankDetail.home.createLabel">Create new Bank Detail</Translate>
          </Link>
        </div>
      </h2>
      <Row>
        ...
        {bankDetailList && bankDetailList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('branchSwiftCode')}>
                  <Translate contentKey="eCompanyApp.bankDetail.branchSwiftCode">Branch Swift Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('branchSwiftCode')} />
                </th>
                <th className="hand" onClick={sort('ibanNo')}>
                  <Translate contentKey="eCompanyApp.bankDetail.ibanNo">Iban No</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('ibanNo')} />
                </th>
            ...
            <tbody>
              {bankDetailList.map((bankDetail, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/bank-detail/${bankDetail.id}`} color="link" size="sm">
                      {bankDetail.id}
                    </Button>
                  </td>
                  <td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/bank-detail/${bankDetail.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/bank-detail/${bankDetail.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="eCompanyApp.bankDetail.home.notFound">No Bank Details found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={bankDetailList && bankDetailList.length > 0 ? '' : 'd-none'}>
